...
def dashboard_chart(orders):
    ...
    created_array = df[df['order_state']==1][df['response_code']!=200][['partner_id','store_id_by_ck','error_message','response_code','last_seen']].to_numpy()
    cancelled_array = df[df['order_state']==2][df['response_code']!=202][['partner_id','store_id_by_ck','error_message','response_code','last_seen']].to_numpy()
